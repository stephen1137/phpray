...
needs no account at all. Eleven tools: sites, overview, slowest pages,
slow SQL, component breakdown, errors, traces, one trace in full, before/after
comparison, alerts, and turning profiling on for a URL prefix. Everything is
read-only except the last one. Source and protocol notes: `src/mcp/`.
