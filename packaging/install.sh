...
install_collector() {
    ...
    if command -v systemctl >/dev/null 2>&1; then
        ...
        if systemctl enable --now phpray-collector 2>/dev/null; then
            ...
            log_warn "systemctl enable --now phpray-collector failed - start it manually"
        fi
    else
        log_warn "systemctl not available - start the collector manually"
    fi
    return 0
}
...
print_next_steps() {
    ...
    printf '%s\n' \
        ...
        "  2. in /etc/phpray/collector.toml set:  [cloud]  enabled = true  endpoint = \"https://app.phpray.dev\"  server_key = \"prk_...\"" \
