...
func cmdTop(args []string) {
	...
	for {
		...
		// Jedna linia o Cloudzie, TYLKO gdy nie jest skonfigurowany. `phpray
		// top` to jedyne, na co patrzy czlowiek pracujacy z terminala, a nie
		...
		if !cloudSkonfigurowany(*cfgPath) {
			...
			// webhook delivery are planned") — to ja wpisalem tu obietnice,
			// ktorej produkt nie dotrzymuje. Wezwanie ma byc prawdziwe nawet
			// wtedy, gdy przez to jest slabsze.
			fmt.Println("  joins your servers into one list and flags failing sites:  https://phpray.dev/en/cloud")
			fmt.Println("  (free for one site; the local core stays unlimited and needs no account)")
		}
...
func sprawdzKolektor(addr string) string {
	...
	if err != nil {
		return "phpray top: the collector is not answering on " + addr + " (" + err.Error() + ")\n" +
			// Bez systemd (kontener, WSL, macOS, czesc hostingow) pierwsza linia
			// jest bezuzyteczna, a uzytkownik nie zgadnie, ze demon z panelem to
			// `serve` z konfiguracja i adresem.
			"  Start it:        systemctl start phpray-collector\n" +
			"  Without systemd: phpray-collector serve -c /etc/phpray/collector.toml -addr " + addr + "\n" +
...
// juz go podlaczyl, nie potrzebuje zaproszenia przy kazdym odswiezeniu.
func cloudSkonfigurowany(cfgPath string) bool {
	cfg, err := LoadConfig(cfgPath)
	if err != nil {
		return false
	}
	return cfg.Cloud.Enabled && cfg.Cloud.ServerKey != ""
}
